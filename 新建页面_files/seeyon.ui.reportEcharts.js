 ...
 * 判断统计数据是否为空数据
 * @param option 
 * return  true 无数据图，false 有数据
 */
function isEmptyData(option){
 ...
 * chartDataStr 图的数据
 * dom 显示的位置
 * fromType 来源类型：区分数据关联，和统计结果
 */
function drawingChart2(chartType,option,dom,fromType){
	var et = null;
	var isEmpty = isEmptyData(option);//是否有数据;
...
// 			toolbox.feature.dataZoom = dataZoom;
		...
		case 7://饼图
				...
				for(var j=0;j<sersTh;j++){
					...
					for(var i=0;i<oldxData.length;i++){
						if(0 != oldsData[i].value){
							newsData[newsData.length] = {"name": name,"value": oldsData[i].value,"display":oldsData[i].display};
						}
					}
				}
			}
			...
			newOption.legend.selectedMode = true;//图例开关
			...
			if("DataRelation" == fromType){
				newOption.legend.selectedMode = false;//图例开关
				...
				var normal = {"label": {"formatter": "{d}%", "position":"outside","distance":0.8,"show": true},"labelLine":{"show":true}};
				//normal.label.formatter = seriesFormatter;//调用seriesFormatter方法，导致百分比不生效
				itemStyle.normal = normal;
				//itemStyle.emphasis = normal;//饼图外显示百分比会出现重复问题
			}
...
function optionDataAdjustment(option,dom){
	...
	var _x = (option.grid && option.grid.x) ||  60;   //直角坐标系内绘图网格左上角横坐标
	var _x2 = (option.grid && option.grid.x2) ||  60; //直角坐标系内绘图网格右下角横坐标
	...
	var _actualWidth = domWidth - _x - _x2; //实际可用宽度
