...
;~(function($) {
	...
	$.fn.touch = function(params) {
		var viewOrginal = $.i18n('common.touchtouch.viewOrginal');
		var loadfailure = $.i18n("common.touchtouch.image.loadfailure");
		var ishowReply = $.i18n("showReply.reply.name");
		// 初始化数据
		...
		function showNext() {
			if (currentIndex + 1 < options.datas.length) {
				...
				if (options.targetWindow.layer_dialog) {
					options.targetWindow.layer_dialog(false, 'action_warning', $.i18n('common.touchtouch.lasttips'), 2, 1);
				}else{
					alert($.i18n('common.touchtouch.lasttips'));
				}
			}
		}
		...
		function loadContent(index){
			...
			if(options.showpostContent || options.userPic){
				...
				if(options.showpostContent){
					var text = $.i18n("common.touchtouch.image.viewdetail");
